...
help:
	...
	echo "  destroy - Delete app cluster and minikube cluster"
	echo "  list - List all packages"
	echo "  create	- Create packages. Usage: make create package=<name_of_the_package>"
	echo "  delete	- Delete packages. Usage: make create id=<id_of_the_package>"
	echo
...
list:
	curl -sb -H "Accept: application/json" "http://challenge-delta.info/packages"

create:
ifdef package
	curl -X POST http://challenge-delta.info/packages --header "Content-Type: text/plain" -d $(package) || true
else
...
delete:
ifdef id
	curl -X DELETE http://challenge-delta.info/packages/$(id) || true
	echo
else
